...
contract property {
  ...
  uint public avail=4000; //sqft mein
  uint cf=1;
  /*
 ...
 function show() view public returns(uint[]){
  ...
  function buy(uint  room_id) public payable {
    ...
    okay= cf * 400 * 1 ether;
    if (buyer != 0x0) {
      buyer.transfer(uint(okay));
    }
    delete bahot_ids[room_id];
  }
 
  function allpropsold() restricted public payable {
    mainwala.transfer(address(this).balance);
    avail=0;
  }
 
  modifier restricted() {	
    require(msg.sender == mainwala);
    _;
  }
}
